...
(function (global) {
  ...
  let presets = [];
  let ghConfig = { token: '', repo: '', folder: 'presets', imgFolder: 'img', legacyPath: 'presets.json' };
  ...
  function loadProjectData(data, opts) {
    ...
    bg = { ...bg, ...(data.bg || {}) };
    if (bg.imageSrc) {
      resolveImageSrc(bg.imageSrc).then(src => {
        if (token !== bgLoadToken || !src) return;
        const img = new Image();
    ...
    layers = (data.layers || []).map(l => {
      ...
      if (layer.type === 'image' && layer.src) {
        resolveImageSrc(layer.src).then(src => {
          if (!src) return;
          const img = new Image();
          img.onload = () => { layer.imgObj = img; render(); };
          img.onerror = () => console.error('failed to load layer image', layer.name);
          img.src = src;
        });
      }
      return layer;
  ...
  // Presets embed full base64 image data (backgrounds, uploaded layer
  // images), so the whole array can easily blow past the browser's
  // localStorage quota (~5-10MB per origin) once a few presets pile up.
  // GitHub (when connected) is the durable, full-fidelity store — this
  // local cache is just a convenience for offline/first-load. So if the
  // full write fails, fall back to a lightweight cache (metadata + thumb,
  // no embedded image data) instead of throwing and silently breaking
  // whatever caller was mid-way through (which previously prevented
  // `emit('presets')` from ever running, so the grid appeared empty/stale).
  function persistPresetsLocal() {
    try {
      localStorage.setItem('tc_presets_v2', JSON.stringify(presets));
      return true;
    } catch (e) {
      console.error('preset local cache exceeded quota, falling back to lightweight cache', e);
      try {
        const light = presets.map(p => ({
          id: p.id, name: p.name, thumb: p.thumb, updatedAt: p.updatedAt,
          _slug: p._slug, _path: p._path, _sha: p._sha, _lightweight: true
        }));
        localStorage.setItem('tc_presets_v2', JSON.stringify(light));
      } catch (e2) {
        console.error('lightweight preset cache also failed, clearing local preset cache', e2);
        try { localStorage.removeItem('tc_presets_v2'); } catch (e3) {}
      }
      emit('toast', { msg: ghConnected()
        ? 'Preset library too large to cache locally — using GitHub as source of truth'
        : 'Preset library too large to cache locally — connect GitHub to avoid losing presets on reload',
        type: 'info' });
      return false;
    }
  }
  ...
  function savePresetNew(name) {
    ...
    scheduleAutosave(); persistPresetsLocal();
    emit('presets');
  ...
  // ── GITHUB-BACKED IMAGES (img/ folder) ──────────────────────────
  // Preset images (backgrounds + image layers) are extracted out of the
  // preset JSON and uploaded as standalone files in ghConfig.imgFolder,
  // referenced from the preset data as 'ghimg://<path>' strings. This
  // keeps preset JSON (and its localStorage cache) small instead of
  // carrying full base64 image data in every preset. Old presets that
  // still have images embedded as base64 keep working exactly as before
  // (resolveImageSrc just passes them through) — they get migrated the
  // next time that preset is saved/updated, or in bulk via "Save All".
  const ghImageCache = new Map(); // path -> blob: URL, resolved once per session
  ...
  function mimeFromPath(path) {
    ...
    return { png: 'image/png', jpg: 'image/jpeg', jpeg: 'image/jpeg', webp: 'image/webp', gif: 'image/gif', svg: 'image/svg+xml' }[ext] || 'application/octet-stream';
  }
  async function ghListImgFolder() {
    try {
      const url = `https://api.github.com/repos/${ghConfig.repo}/contents/${ghConfig.imgFolder || 'img'}`;
  ...
  // "My Thumbnail.png", then "My Thumbnail (1).png", "My Thumbnail (2).png", ...
  function uniqueImageName(baseName, ext, existingNames) {
    const clean = (baseName || 'image').toString().trim().replace(/[\\/:*?"<>|]+/g, '').replace(/\s+/g, ' ').trim() || 'image';
    let name = `${clean}.${ext}`, n = 1;
    while (existingNames.has(name)) { name = `${clean} (${n}).${ext}`; n++; }
    existingNames.add(name);
    return name;
  }
  async function ghUploadImage(dataUrl, baseName, existingNames) {
    const ext = extFromDataUrl(dataUrl);
    const content = (dataUrl.split(',')[1]) || '';
    const filename = uniqueImageName(baseName, ext, existingNames);
    const path = `${ghConfig.imgFolder || 'img'}/${filename}`;
    const url = `https://api.github.com/repos/${ghConfig.repo}/contents/${path}`;
    const put = await fetch(url, { method: 'PUT', headers: ghHeaders(), body: JSON.stringify({ message: `ThumbCraft: add image "${filename}"`, content }) });
    if (!put.ok) { const err = await put.json().catch(() => ({})); throw new Error(err.message || ('image upload failed: ' + put.status)); }
    ghImageCache.set(path, dataUrl); // we already have the bytes locally — skip re-fetching this session
    return `ghimg://${path}`;
  }
  // Walks a preset's data and uploads any embedded base64 images to the
  // img/ folder, replacing them with 'ghimg://path' references in place.
  // Mutates and returns p (safe — p.data is always an independent clone,
  // never the live canvas state; see serializeProject/stripRuntime).
  async function extractPresetImages(p) {
    if (!ghConnected() || !p || !p.data) return p;
    const data = p.data;
    const needsBg = data.bg && data.bg.imageSrc && data.bg.imageSrc.startsWith('data:');
    const needsLayers = Array.isArray(data.layers) && data.layers.some(l => l.type === 'image' && l.src && l.src.startsWith('data:'));
    if (!needsBg && !needsLayers) return p;
    const existingNames = new Set(await ghListImgFolder());
    if (needsBg) {
      try {
        const ref = await ghUploadImage(data.bg.imageSrc, `${p.name} bg`, existingNames);
        data.bg = { ...data.bg, imageSrc: ref };
      } catch (e) {
        console.error('failed to upload background image to GitHub', e);
        emit('toast', { msg: `Couldn't upload "${p.name}" background to GitHub — kept it embedded`, type: 'error' });
      }
    }
    if (needsLayers) {
      for (let i = 0; i < data.layers.length; i++) {
        ...
        if (l.type === 'image' && l.src && l.src.startsWith('data:')) {
          try {
            const ref = await ghUploadImage(l.src, `${p.name} - ${l.name || 'image'}`, existingNames);
            data.layers[i] = { ...l, src: ref };
          } catch (e) {
            console.error('failed to upload layer image to GitHub', e);
            emit('toast', { msg: `Couldn't upload "${l.name || 'image'}" to GitHub — kept it embedded`, type: 'error' });
          }
        }
      }
    }
  ...
  async function ghFetchImageBlob(path) {
    if (ghImageCache.has(path)) return ghImageCache.get(path);
    const sha = await ghGetFileSha(path);
    if (!sha) throw new Error('Image not found in GitHub: ' + path);
    const r = await fetch(`https://api.github.com/repos/${ghConfig.repo}/git/blobs/${sha}`, { headers: ghHeaders() });
    if (!r.ok) throw new Error('Could not fetch image from GitHub');
    const j = await r.json();
    const blob = new Blob([b64ToBytes(j.content)], { type: mimeFromPath(path) });
    const url = URL.createObjectURL(blob);
    ghImageCache.set(path, url);
    return url;
  }
  // Resolves a stored image src to something an <img> element can load:
  // legacy base64 (or any plain URL) passes straight through unchanged;
  // 'ghimg://path' references are fetched from GitHub and turned into a
  // local blob: URL. This is what lets old presets keep loading exactly
  // as before while new/re-saved ones use the lightweight reference form.
  function resolveImageSrc(src) {
    if (!src) return Promise.resolve(src);
    if (src.startsWith('ghimg://')) {
      const path = src.slice('ghimg://'.length);
      return ghFetchImageBlob(path).catch(e => {
        console.error('failed to resolve GitHub image', path, e);
        emit('toast', { msg: `Couldn't load image "${path}" from GitHub`, type: 'error' });
        return '';
      });
    }
  ...
  async function ghTestAndConnect(token, repo, folder) {
    ...
    ghConfig = { token, repo, folder: folder || 'presets', imgFolder: ghConfig.imgFolder || 'img', legacyPath: ghConfig.legacyPath || 'presets.json' };
    saveGhConfigLocal();
    return true;
  }
  function ghDisconnect() { ghConfig = { token: '', repo: '', folder: 'presets', imgFolder: 'img', legacyPath: 'presets.json' }; localStorage.removeItem('tc_gh_v2'); localStorage.removeItem('tc_gh'); emit('ghstatus'); }
  ...
  async function ghSavePresetFile(p) {
    ...
    try {
      await extractPresetImages(p); // swap embedded base64 images for ghimg:// refs, uploading to img/
      persistPresetsLocal(); // local cache now reflects the smaller, reference-based data too
      ...
      if (put.ok) { const j = await put.json(); p._sha = j.content && j.content.sha; p._path = path; persistPresetsLocal(); emit('toast', { msg: `✓ Synced "${p.name}" to GitHub`, type: 'success' }); }
